<div class="wrap">
	<h1 class="wp-heading-inline"><?php echo esc_html_e( 'Amount Payment', 'amount-payment' ); ?></h1>
	...
	<form action="options.php" method="post">
		...
		<fieldset style="padding:10px 0;">
			...
			<label for="amount_payment_product">
				<span><?php esc_attr_e( 'Choose a product to add to cart for amount payment.', 'amount-payment' ); ?></span>
				<br />
				<select name="amount_payment_settings[amount_payment_product]" id="amount_payment_product">
					<?php foreach ( $products as $product ) : ?>
						<option value="<?php echo esc_attr( $product->get_id() ); ?>" <?php echo ( isset( $options['amount_payment_product'] ) && intval( $options['amount_payment_product'] ) === $product->get_id() ) ? 'selected="selected"' : ''; ?>> <?php echo esc_html( $product->get_name() ); ?></option>
					<?php endforeach; ?>
				</select>
		...
		<fieldset style="padding:10px 0;">
			...
			<label for="amount_payment_auto_complete">
				...
				<span><?php esc_attr_e( 'Enable auto complete order for new orders.', 'amount-payment' ); ?></span>
			</label>
		</fieldset>
		<p>
			<input class="button-primary" type="submit" value="<?php echo esc_html_e( 'Save', 'amount-payment' ); ?>">
		</p>
